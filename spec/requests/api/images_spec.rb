...
RSpec.describe 'ImagesAPI', type: :request do
  ...
  describe 'GET /images/' do
    it 'show all images info per page' do
      get "/api/v1/images/#{image.id}", params: { page: 1 }, headers: { 'HTTP_TOKEN_USER' => user.api_token.to_s }
      expect(response).to have_http_status(200)
      expect(response).to be_present
    end
  end
  ...
  describe 'post /images/' do
    it 'create image' do
      post '/api/v1/images', params: { image: fixture_file_upload('files/index.jpeg', 'image/jpeg') }, headers: { 'HTTP_TOKEN_USER' => user.api_token.to_s }
      expect(response).to be_present
      expect(response).to have_http_status(200)
    end
    ...
    it 'not create image because of invalid data' do
      post '/api/v1/images', params: { image: 'dfdfdfdf' },
                             headers: { 'HTTP_TOKEN_USER' => user.api_token.to_s }
      result = JSON.parse response.body
  ...
  describe 'PUT /images/like' do
    it 'create like to image' do
      put '/api/v1/images/like', params: { id: image.id },
                                 headers: { 'HTTP_TOKEN_USER' => user.api_token.to_s }
      result = JSON.parse response.body
      expect(result['image']).to be_present
      expect(response).to have_http_status(200)
  ...
  describe 'PUT /images/dislike' do
    it 'create dislike to image' do
      create(:like, user_id: 1, image_id: image.id)
      put '/api/v1/images/dislike', params: { id: image.id },
                                    headers: { 'HTTP_TOKEN_USER' => user.api_token.to_s }
      result = JSON.parse response.body
      expect(result['image']).to be_present
      expect(response).to have_http_status(200)
    ...
    it 'not dislike to not liked image' do
      ...
      put '/api/v1/images/dislike', params: { id: 3 },
                                    headers: { 'HTTP_TOKEN_USER' => user.api_token.to_s }
      result = JSON.parse response.body
