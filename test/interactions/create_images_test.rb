...
describe CreateImages do
  before do
    # image = fixture_file_upload('files/index.jpeg', 'image/jpeg')
  	@result_true = CreateImages.run(params: { image: File.open("/home/ignatiy/Загрузки/index.jpeg",'r'),
                                           title_img: 'testing title',
                                           tags: 'testing title1',
                                           user_id: User.first.id })
    @result_false = CreateImages.run(params: { image: File.open("/home/ignatiy/Загрузки/ngrok-stable-linux-amd64.zip",'r'),
                                           title_img: 'testing title',
                                           tags: 'testing title1',
                                           user_id: User.first.id })
  end
