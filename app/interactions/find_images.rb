...
class FindImages < ActiveInteraction::Base
  ...
  def execute
    ...
    images = images_all.where("title_img LIKE ? or tags LIKE ?" , "%#{params[:condition_search]}%","%#{params[:condition_search]}%") unless params[:condition_search].empty?
    images = images.reorder(created_at: :DESC) if params[:sort_data]
    images = images.reorder(cached_votes_up: :DESC) unless params[:sort_data]
